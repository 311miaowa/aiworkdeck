...
export function loadWpsSDK() {
    return new Promise((resolve, reject) => {
        ...
        if (typeof window !== "undefined") {
            ...
            // 生产环境：需要配置正确的静态资源路径
            const sdkPath = import.meta.env.DEV ? "/static/web-office-sdk-solution-v2.0.7.umd.js" : "./web-office-sdk-solution-v2.0.7.umd.js";
            script.src = sdkPath;
            script.onload = () => {
...
export async function initWpsEditor(options) {
    ...
    console.log("[WPS-SDK] 初始化WPS编辑器:", { appId, fileId, fileName, officeType, mode, token: token ? "(provided)" : "(none)" });
    ...
    const initConfig = {
        ...
        onHyperLinkOpen: (payload) => {
            try {
                // 官方示例字段：linkUrl
                // https://solution.wps.cn/docs/demo/public/intercept-link.html#%E8%B6%85%E9%93%BE%E6%8E%A5%E8%B7%B3%E8%BD%AC%E6%8B%A6%E6%88%AA
                const linkUrl = (payload && (payload.linkUrl || payload.LinkUrl || payload.url || payload.URL || payload.href || payload.Href)) || "";
    ...
    console.log("[WPS-SDK] 调用 SDK.init...");
    const instance = SDK.init(initConfig);
    console.log("[WPS-SDK] SDK.init 返回 instance:", instance ? "OK" : "NULL");

    // 等待编辑器就绪
    console.log("[WPS-SDK] 等待 instance.ready()...");
    try {
        await instance.ready();
        console.log("[WPS-SDK] instance.ready() 成功!");
    } catch (readyError) {
        console.error("[WPS-SDK] instance.ready() 失败:", readyError);
        throw readyError;
    }

    // 监听文件打开事件
    instance.ApiEvent.AddApiEventListener("fileOpen", (data) => {
        console.log("[WPS-SDK] 文件打开事件:", data);
        if (data.success) {
            console.log("[WPS-SDK] 文件打开成功:", data.fileInfo);
        } else {
            console.error("[WPS-SDK] 文件打开失败:", data.msg, data);
        }
    });
