...
def export_editable_pptx(project_id):
    ...
    try:
        ...
        if model_config:
            logger.info(f"Received model_config for editable export: provider={model_config.get('provider')}, image_model={model_config.get('image_model')}")
        else:
            logger.info("No model_config provided, will use default AI service configuration")
        ...
        file_service = FileService(current_app.config['UPLOAD_FOLDER'])
        # 使用传递的模型配置创建 AIService（如果提供了）
        # 注意：这里只是为了记录日志，实际 AIService 在后台任务中按需创建
        if model_config:
            logger.info(f"Will use dynamic model config for clean background generation")
        ...
        # 传递 model_config 以便在后台任务中使用正确的模型生成干净背景图
