...
class ExportService:
    ...
    # ==================== PPTX Slide Image Extraction and Replacement ====================

    @staticmethod
    def extract_slide_image(pptx_path: str, slide_index: int, output_path: str = None) -> Optional[str]:
        """
        Extract the main image from a slide (for image-only slides).
        ...
        Returns:
            Path to the extracted image, or None if no image found
        """
        try:
            prs = Presentation(pptx_path)
            
            if slide_index < 1 or slide_index > len(prs.slides):
                logger.error(f"Invalid slide index: {slide_index}, total slides: {len(prs.slides)}")
                return None
            
            slide = prs.slides[slide_index - 1]  # Convert to 0-based
            
            # Find the largest image shape on the slide
            largest_image = None
            largest_area = 0
            ...
            if not largest_image:
                logger.warning(f"No image found on slide {slide_index}")
                return None
            
            # Extract the image data
            image_blob = largest_image.image.blob
            image_ext = largest_image.image.ext  # e.g., 'png', 'jpeg'
            
            # Determine output path
            if not output_path:
                output_path = tempfile.mktemp(suffix=f'.{image_ext}')
            
            # Save the image
            with open(output_path, 'wb') as f:
                f.write(image_blob)
            
            logger.info(f"Extracted slide {slide_index} image to: {output_path}")
            return output_path
            
        except Exception as e:
            logger.error(f"Failed to extract slide image: {str(e)}", exc_info=True)
            return None
    
    @staticmethod
    def replace_slide_image(pptx_path: str, slide_index: int, new_image_path: str, 
                            output_path: str = None) -> Optional[str]:
        """
        Replace the main image on a slide with a new image.
        
        Args:
            pptx_path: Path to the PPTX file
            slide_index: Slide index (1-based)
            new_image_path: Path to the new image
            output_path: Optional output path for the modified PPTX (defaults to overwrite original)
        
        Returns:
            Path to the modified PPTX file, or None if failed
        """
        try:
            prs = Presentation(pptx_path)
            ...
            if not pictures:
                logger.warning(f"No image found on slide {slide_index}, will add new image")
                # Add the new image to fill the entire slide
                slide.shapes.add_picture(
                    new_image_path,
                    left=0,
                    top=0,
                ...
                # Sort by area (largest first) and replace the largest image
                pictures.sort(key=lambda x: x['area'], reverse=True)
                main_pic = pictures[0]
                
                # Remove the old shape
                sp = main_pic['shape']._element
                sp.getparent().remove(sp)
                
                # Add the new image at the same position
                slide.shapes.add_picture(
                    new_image_path,
                    left=main_pic['left'],
                    top=main_pic['top'],
            ...
            logger.info(f"Saved modified PPTX to: {output_path}")
            return output_path
            
        except Exception as e:
            logger.error(f"Failed to replace slide image: {str(e)}", exc_info=True)
            return None
    
    @staticmethod
    def edit_slide_with_ai(pptx_path: str, slide_index: int, edit_instruction: str,
                           ai_service, output_path: str = None) -> Tuple[bool, str, Optional[str]]:
        """
        Complete workflow: extract slide image -> AI edit -> replace back.
        ...
        Args:
            ...
            ai_service: AIService instance for image editing
            output_path: Optional output path for the modified PPTX
        
        Returns:
            Tuple of (success: bool, message: str, output_path: Optional[str])
        """
        try:
            # Step 1: Extract the slide image
            logger.info(f"Step 1: Extracting image from slide {slide_index}")
            extracted_image = ExportService.extract_slide_image(pptx_path, slide_index)
            
            if not extracted_image:
                return False, f"Slide {slide_index} does not contain an image", None
            
            # Step 2: Edit the image with AI
            logger.info(f"Step 2: Editing image with AI instruction: {edit_instruction[:50]}...")
            
            edited_image = ai_service.edit_image(
                prompt=edit_instruction,
                current_image_path=extracted_image,
                aspect_ratio="16:9",
                resolution="2K",
                original_description=None,
                additional_ref_images=None
            ...
            if not edited_image:
                return False, "AI failed to edit the image", None
            ...
            if not isinstance(edited_image, Image.Image):
                if hasattr(edited_image, '_pil_image'):
                    ...
                    return False, f"Unexpected image type from AI: {type(edited_image)}", None
            
            # Save edited image to temp file
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
                edited_image_path = tmp.name
                if edited_image.mode in ('RGBA', 'LA', 'P'):
                    edited_image = edited_image.convert('RGB')
                edited_image.save(edited_image_path, format='PNG')
            
            logger.info(f"Step 3: Replacing slide image with edited version")
            
            # Step 3: Replace the slide image
            result_path = ExportService.replace_slide_image(
                pptx_path, slide_index, edited_image_path, output_path
            )
            
            # Clean up edited image temp file
            if os.path.exists(edited_image_path):
                os.remove(edited_image_path)
            
            if not result_path:
                return False, "Failed to replace the slide image", None
            
            return True, f"Successfully edited slide {slide_index}", result_path
            
        except Exception as e:
            logger.error(f"Failed to edit slide with AI: {str(e)}", exc_info=True)
